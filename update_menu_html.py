...
file_path = 'dabeli/templates/menu.html'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Replace filters menu
filter_pattern = r'<ul class="filters_menu">.*?</ul>'
filter_replacement = """<ul class="filters_menu">
...
# Replace filters content
filters_content_pattern = r'<div class="filters-content">.*?</div>\s*</div>\s*</div>'
# Wait, the closing tags might be tricky. Let's just find the start and end of the div.
filters_content_start = content.find('<div class="filters-content">')
filters_content_end = content.find('</section>', filters_content_start)

if filters_content_start != -1 and filters_content_end != -1:
    ...
    replacement = """<div class="filters-content">
        <div class="row grid">
          {% for item in items %}
          <div class="col-sm-6 col-lg-4 all {{ item.category.css_class }}">
            <div class="box">
              <div>
                <div class="img-box">
...
"""
    ...
    content = content[:filters_content_start] + replacement + "    " + content[filters_content_end:]
